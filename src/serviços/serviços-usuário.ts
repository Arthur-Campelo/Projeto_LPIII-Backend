...
export default class ServiçosUsuário {
    ...
    static async verificarCnpjExistente(request, response) {
        try {
            const cnpj_encriptado = md5(request.params.cnpj);
            const usuário = await Usuário.findOne(cnpj_encriptado);
            if (usuário) return response.status(404).json({ erro: "CNPJ já cadastrado." });
            else return response.json();
        } catch (error) {
            return response.status(500).json({ erro: "Erro BD: verificarCnpjCadastrado" });
        }
    };
    ...
    static async verificarCadastroCompleto(usuário: Usuário) {
        switch (usuário.perfil) {
            ...
            case Perfil.LOCADORA_MOTOS:
                const locadoraMotos = await LocadoraMotos.findOne({
                    where: { usuário: usuário.cnpj },
                    relations: ["usuário"]
                });
                if (!locadoraMotos) return false;
                return true;

            case Perfil.ORGANIZADOR_EVENTOS_MOTOS:
                const organizadorEventosMotos = await OrganicadorEventosMotos.findOne({
                    where: { usuário: usuário.cnpj },
                    relations: ["usuário"]
                });
                if (!organizadorEventosMotos) return false;
                return true;
    ...
    static async logarUsuário(request, response) {
        try {
            ...
            const cnpj_encriptado = md5(nome_login);
            const usuário = await Usuário.findOne(cnpj_encriptado);
    ...
    static async cadastrarUsuário(usuário_informado) {
        try {
            const { cnpj, nome, perfil, email, senha, questão, resposta, cor_tema } = usuário_informado;
            ...
            const usuário = Usuário.create({
                cnpj: cnpj_encriptado, nome, perfil, email,
